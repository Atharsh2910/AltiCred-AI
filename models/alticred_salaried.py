...
class AltiCredScorer:
    ...
    def _train_all_models(self):
        ...
        self._train_language_sentiment_model() # This will now use VADER
    ...
    def _train_language_sentiment_model(self):
        # --- 2. MODIFIED: Training now uses VADER to generate sentiment scores ---
        # This ensures the model is trained on the same features it will predict with.
        ...
        # Generate sentiment scores from the posts in the training data
        ...
        # Use the new generated score as the sentiment feature
    ...
    def _predict_language_sentiment_score(self, data):
        # --- 3. MODIFIED: Prediction now uses VADER automatically ---
        df = data.copy().reindex(columns=self.df.columns, fill_value=0)
        df['cleaned_posts'] = df['user_posts'].apply(lambda x: re.sub(r'[^\w\s]', '', str(x).lower()))
        
        # Automatically generate the sentiment score from the input post
        get_score = lambda text: self.sentiment_analyzer.polarity_scores(text)['compound']
    ...
    def predict_alticred_score(self, user_data):
        ...
        # These scores are now all the probability of DEFAULT (class 1)
        ...
        # 1. CORRECTED: Get the meta-model's probability of DEFAULT (class 1)
        meta_prediction_proba = self.meta_model.predict_proba(base_scores)[0][1]
        
        # The base_scores are already the risks (probability of default)
        risks = base_scores.flatten()
        total_risk = np.sum(risks)
        
        # 2. CORRECTED: The weights should now be based directly on the risk scores
        weights = risks / total_risk if total_risk > 0 else np.array([0.25, 0.25, 0.25, 0.25])
            
        # The weighted score is an average of the risks, weighted by the risks themselves
        weighted_score = np.sum(risks * weights)
        
        # Combine the meta-model's prediction and the weighted average
        final_score = (0.5 * meta_prediction_proba) + (0.5 * weighted_score)
        
        return np.clip(final_score, 0, 1)

if __name__ == '__main__':
    # You must have the data file for training. If not available, this will fail.
    # You may need to adjust the file_path in the AltiCredScorer constructor.
    ...
    while True:
        try:
            user_input = {
                ...
                # --- 4. REMOVED: No longer need to ask the user for a sentiment score ---
